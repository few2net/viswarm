  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  dxl_efforts_.resize(info_.joints.size(), 0.0);
...
TurtleBot3ManipulationSystemHardware::export_state_interfaces()
{
  ...
  for (uint8_t i = 0; i < info_.joints.size(); i++) {
    ...
    state_interfaces.emplace_back(
      hardware_interface::StateInterface(
        info_.joints[i].name, hardware_interface::HW_IF_EFFORT, &dxl_efforts_[i]));
  }
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  dxl_efforts_[0] = opencr_->get_wheel_efforts()[opencr::wheels::LEFT];
  ...
  dxl_efforts_[1] = opencr_->get_wheel_efforts()[opencr::wheels::RIGHT];
  ...
  dxl_efforts_[2] = opencr_->get_joint_efforts()[opencr::joints::JOINT1];
  ...
  dxl_efforts_[3] = opencr_->get_joint_efforts()[opencr::joints::JOINT2];
  ...
  dxl_efforts_[4] = opencr_->get_joint_efforts()[opencr::joints::JOINT3];
